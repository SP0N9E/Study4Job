...
public:
    ...
    TreeNode *deserialize(string data)
    {
        ...
        int idx = 0;
        return decodeHelper(vec, idx);
    }

    TreeNode *decodeHelper(const vector<string>& data, int &idx)
    {
        const string &val = data.at(idx);
        ++idx;
        if (val == "#")
        {
            return nullptr;
        ...
        node->left = decodeHelper(data, idx);
        node->right = decodeHelper(data, idx);
